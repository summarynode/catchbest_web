    ...
    def do_GET(self):
        ...
        if parsed_path.path == "/login":
           ...
           if len(msg) == 0:
              ...
              #print 'set size [%s] [%d] [%s]' % (t_key, len(t_value), t_value)
           ...
           if len(msg) == 0:
              ...
              if len(str(result_value)) == 0:
                 result_value = "key is None"

           self.send_response(200)
           encoding = sys.getfilesystemencoding()
           self.send_header("Content-type", "text/html; charset=%s" % encoding)
           self.send_header("Content-Length", str(len("success:" + str(result_value))))
           self.end_headers()

           self.wfile.write("success:" + str(result_value))
...
if __name__ == '__main__':
    try:
       ...
       server = ThreadedHTTPServer(('', 7777), Handler)
